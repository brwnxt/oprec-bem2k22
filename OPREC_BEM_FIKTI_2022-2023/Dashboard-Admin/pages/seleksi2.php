...
$seleksi_final = mysqli_query($conn,"SELECT * FROM users_account 
                              INNER JOIN biodata USING(email) 
                              INNER JOIN users_study_data USING(email) 
                              INNER JOIN users_choose_point USING(email) 
                              WHERE users_account.berkas = 2");

//cek link selesai sudah ditekan atau belum
if( isset($_GET["email"]) && isset($_GET["seleksi2"])){
  //cek data berhasil diupdate atau tidak
  if( seleksiFinal($_GET) > 0 ){ 
    echo "
      <script>
        alert('Status Seleksi Final Berhasil Diubah');
        document.location.href = 'seleksi2.php';
      </script>
    ";
  } else {
    echo "
      <script>
        alert('Status Seleksi Final Gagal Diubah');
        document.location.href = 'seleksi2.php';
      </script>
    ";
  }
...
<html lang="en">
  <head>
    <title>Seleksi Final OPREC 2021</title>
            ...
            <!-- content for nav link #daftar_menu start-->
              ...
              <table id="example" class="table table-bordered table-hover dt-responsive nowrap text-center" style="width:100%; background-color:#fff;">
                ...
                <tr>
                    ...
                    <th>Pilihan 1</th>
                    <th>Pilihan 2</th>
                    <th>Status Seleksi Final</th>
                    <th>Aksi</th>
                </tr>
                </thead>
                <?php
                  $no = 1;
                  foreach($seleksi_final as $seleksi2) :
                ?>
                  <tr>
                    <td><?= $no;?></td>
                    <td><?= $seleksi2['nama_lengkap']?></td>
                    <td><?= $seleksi2['npm'];?></td>
                    <td><?= $seleksi2['choose_one'];?></td>
                    <td><?= $seleksi2['choose_two'];?></td>
                    <td>
                    <?php
                    switch($seleksi2['final']) {
                      case 1 :
                    ?>
                    ...
                    <td>
                      <a href="?email=<?= $seleksi2['email']; ?>&&seleksi2=1" onclick="return confirm('Yakin <?= $seleksi2['nama_lengkap']. ' ('. $seleksi2['npm']. ')' ?>  Tidak Lulus Seleksi Final?');"> <!-- URL seleksi1 = 1= tidak lulus-->
                        <button type="button" class="btn btn-gradient-danger btn-rounded btn-icon btn-sm mr-2">
                          <i class="mdi mdi-close"></i>
                        </button>
                      </a>
                      <a href="?email=<?= $seleksi2['email']; ?>&&seleksi2=2" onclick="return confirm('Yakin <?= $seleksi2['nama_lengkap']. ' ('. $seleksi2['npm']. ')' ?>  Lulus Seleksi Final?');"> <!-- URL seleksi= 2= lulus-->
